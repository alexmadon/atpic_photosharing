...
def whitespace(ss):
    """
    Returns a string with whitespaces after and before each chinese character
    """
    sse=ss.decode('utf8')
    RE = re.compile('([⺀-⺙⺛-⻳⼀-⿕々〇〡-〩〸-〺〻㐀-䶵一-鿃豈-鶴侮-頻並-龎])')
    sse=RE.sub(' \\1 ',sse)
    return sse.encode('utf8')

if __name__ == "__main__":
    alist=['个性化首页 · 网络历史记录','alex','alex 个性化首页 · 网络历史记录','食べました',"私の名前は中野です"]
    for s in alist:
