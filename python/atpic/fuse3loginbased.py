...
# NOT USED ANYMORE:
#  we do not do webdav
# for ftp passwd we use zmq + pam
"""
we import our own fuse * and ctypes *
This filesystem is used to store the HTTP Basic credentials
We do not need write access to it
as passwords are updated usinbg the web interface

We need read access only.
"""

# cat /atpicloginbased/alex
# stores alexmadon:$xxxxpass
# cat /atpicloginbased/servershort
# login:pass
...
xx=atpic.log.setmod("INFO","fuse3loginbased")
...
def my_getattr(path):
    ...
    if path==b'/':
        ...
        uname=path[1:]
        (uidINFO,partition,serverip,login,password)=atpic.pathbased.get_uname_details_lock(uname,lock,rediscon)
        size=len(login+b':'+password)
...
def myread(path, buf, size, offset, fip):
    ...
    atpic.log.debug(yy,'getting password string for path',path)
    uname=path[1:]
    atpic.log.debug(yy,'getting password string for dns uname',uname)

    (uid,partition,serverip,login,password)=atpic.pathbased.get_uname_details_lock(uname,lock,rediscon)


    ret=login+b':'+password
