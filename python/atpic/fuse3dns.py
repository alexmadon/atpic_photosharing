...
we write the IP in a filesystem
Any programming language can access this even erlang, no db driver necessary
"""
...
xx=atpic.log.setmod("INFO","fuse3dns")
...
def my_getattr(path):
    ...
    if path==b'/':
        ...
        # set the size:
        uname=path[1:]
        uname=uname.split(b'.')[0] # take alex in alex.atpic.com or in alex
        atpic.log.debug(yy,'getting IP string for dns uname',uname)
...
def myread(path, buf, size, offset, fip):
    ...
    atpic.log.debug(yy,'getting IP string for path',path)
    uname=path[1:]
    uname=uname.split(b'.')[0] # take alex in alex.atpic.com or in alex
    atpic.log.debug(yy,'getting IP string for dns uname',uname)
