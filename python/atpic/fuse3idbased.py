...
this is read-only:
see idbased.py for details of use of this filesystem
"""
import os
from ctypes import *
...
xx=atpic.log.setmod("INFO","fuse3idbased")
...
def idpath2storepath_wrap(path):
    yy=atpic.log.setname(xx,'idpath2storepath_wrap')
    # get the uname
    atpic.log.debug(yy,'input',path)
    splitted=path.split(b'/')
    atpic.log.debug(yy,splitted)
    (empty,uname,fname)=splitted
    (uid,partition,serverip,login,password)=get_uname_details(uname)
    (isvalid,storepath)=atpic.idbased.idpath2storepath(path,uid,partition) # this also tests permission
    return storepath
...
def my_getattr(path):
    ...
    if slashes<2:
        ...
        storepath=idpath2storepath_wrap(path) # this also test permission
...
def myopen(path, fip):
    ...
    storepath=idpath2storepath_wrap(path) # this also tests permission
